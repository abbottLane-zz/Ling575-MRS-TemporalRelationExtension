...
"""%prog [--help] <e1> <e1_begin> <e1_end> <e2> <e2_begin> <e2_end> [<sent_file>]\n"""
...
def extract_features(e1, e1_b, e1_e, e2, e2_b, e2_e, ace_result):
    ...
    ep_1, ep_2 = None, None
    for ep in mrs.eps():
        if ep.cfrom <= e1_b < ep.cto:
            ep_1 = ep
            continue
        elif ep.cfrom <= e2_b < ep.cto:
            ep_2 = ep
            break
    if ep_1 and ep_2:
        ...
        for prop in mrs.properties(ep_2.iv):
            feat = "e2:" + prop + ":" + mrs.properties(ep_1.iv)[prop]
            features.append(feat)
